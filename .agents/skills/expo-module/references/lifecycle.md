...
For hooking into AppDelegate events without editing AppDelegate directly. Requires app's AppDelegate to extend
`ExpoAppDelegate`.
...
For hooking into Activity/Application lifecycle outside module definitions. Useful for handling deep links, intents, and
...
> Note: `onStart` and `onStop` are **not supported** — the implementation hooks into `ReactActivityDelegate` which lacks
> these methods.
