...
EAS Metadata automates App Store presence management from the command line using a `store.config.json` file instead of
manually filling forms in App Store Connect. It includes built-in validation to catch common rejection pitfalls.
...
Localize metadata for each target market. Keywords should be researched per locale—direct translations often miss
...
```json
{
  "info": {
    ...
    "es-ES": {
      ...
      "keywords": [
        "presupuesto,finanzas,dinero,gastos,ahorro"
      ]
    },
    "ja": {
    ...
    "de-DE": {
      ...
      "keywords": [
        "budget,finanzen,geld,ausgaben,sparen"
      ]
    }
  }
}
