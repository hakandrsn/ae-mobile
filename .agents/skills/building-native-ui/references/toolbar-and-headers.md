...
Add native iOS toolbar items to Stack screens. Items can be placed in the header (left/right) or in a bottom toolbar
area.
...
**Props:** `icon`, `image`, `onPress`, `disabled`, `hidden`, `variant` (`"plain"` | `"done"` | `"prominent"`),
`tintColor`
...
When creating a palette with dividers, use `inline` combined with `elementSize="small"`. `palette` will not apply
dividers on iOS 26.
...
Embed custom React Native components. When adding a custom view make sure that there is only a single child with *
...
- When using `Stack.Toolbar`, make sure that all `Stack.Toolbar.*` components are wrapped inside `Stack.Toolbar`
  component.
