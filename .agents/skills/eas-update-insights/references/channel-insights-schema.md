...
Complete JSON output shape returned by
`eas channel:insights --channel <name> --runtime-version <version> --json --non-interactive`.
...
```json
{
  ...
  "cumulativeMetricsAtLastTimestamp": [
    {
      "id": "...",
      "label": "Embedded update",
      "data": 12345
    },
    {
      "id": "...",
      "label": "Embedded update failed installs",
      "data": 0
    }
  ],
  "uniqueUsersOverTime": {
...
- `otaTotalUniqueUsers` is a sum of `mostPopularUpdates[].totalUniqueUsers`. If more OTA updates are active than the
  top-N the server returns, this figure undercounts true OTA reach.
- A user running the same publish on both iOS and Android may be counted on each platform. Don't treat `uniqueUsers` as
  cross-platform-deduped.
