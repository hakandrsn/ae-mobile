...
export function AppScreen({
  ...
  const isScrollHideEnabled =
    !!tabBarVisibility && !tabBarVisibility.isScrollHideLocked;
  ...
  const onScroll = useAnimatedScrollHandler({
    ...
    onEndDrag: () => {
      ...
      const shouldHide =
        tabBarVisibility.translateY.value > tabBarVisibility.tabBarHeight / 2;
  ...
  const content = (
    <View
      style={[
        styles.content,
        tabBarVisibility && styles.contentWithTabBar,
        contentStyle,
      ]}
    >
      {children}
